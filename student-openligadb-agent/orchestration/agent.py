...
from tools.domain import find_team_matches, get_league_table, serialize_domain_records
from tools.knowledge import lookup_knowledge


class TeamMatchesArgs(BaseModel):
    team_name: str = Field(min_length=1, description="Team name, e.g. Hamburg, Bayern, Dortmund.")
    season: int = Field(default=2025, ge=2000, le=2100)
    league_shortcut: str = Field(default="bl1", min_length=2, max_length=16)
    limit: int = Field(default=5, ge=1, le=20)


class LeagueTableArgs(BaseModel):
    season: int = Field(default=2025, ge=2000, le=2100)
    league_shortcut: str = Field(default="bl1", min_length=2, max_length=16)
    limit: int = Field(default=5, ge=1, le=20)
...
class TemplateAgent:
    ...
    def __init__(self, model: str, vector_store: TemplateVectorStore, session: Any | None = None):
        self.model = model
    ...
    def run(self, user_query: str, max_results: int = 5) -> AgentResult:
        self._latest_records = []
        self._runtime_traces = []
        self._max_results = max(1, min(max_results, 20))

        run_result = Runner.run_sync(self._agent, user_query, session=self.session)
    ...
    def _build_agent(self) -> SdkAgent:
        @function_tool
        def find_team_matches_tool(
            team_name: str,
            season: int = 2025,
            league_shortcut: str = "bl1",
            limit: int = 5,
        ) -> dict[str, Any]:
            """Find matches for one team in an OpenLigaDB league season."""
            try:
                args = TeamMatchesArgs.model_validate(
                    {
                        "team_name": team_name,
                        "season": season,
                ...
                payload = {"error": "Invalid arguments for find_team_matches_tool", "details": exc.errors()}
                self._runtime_traces.append(
                    AgentTrace(
                        tool_name="find_team_matches_tool",
                        arguments={
                            "team_name": team_name,
                            "season": season,
                            "league_shortcut": league_shortcut,
        ...
        @function_tool
        def get_league_table_tool(season: int = 2025, league_shortcut: str = "bl1", limit: int = 5) -> dict[str, Any]:
            """Get league table rows from OpenLigaDB."""
            try:
                args = LeagueTableArgs.model_validate(
                    {
                        "season": season,
                        "league_shortcut": league_shortcut,
                        "limit": limit,
                ...
                payload = {"error": "Invalid arguments for get_league_table_tool", "details": exc.errors()}
                self._runtime_traces.append(
                    AgentTrace(
                        tool_name="get_league_table_tool",
                        arguments={
                            "season": season,
                            "league_shortcut": league_shortcut,
                            "limit": limit,
        ...
        return SdkAgent(
            name="OpenLigaDB Student Agent",
            model=self.model,
            instructions=SYSTEM_PROMPT,
            tools=[find_team_matches_tool, get_league_table_tool, lookup_knowledge_tool],
        )
